...
async function seedInitialAdmin() {
  ...
  await pool.query(
    `INSERT INTO ${USERS_TABLE} (id, username, password_hash, name, role) VALUES ($1,$2,$3,$4,$5)`,
    [newUid(), username, hash, 'Administrateur', 'admin']
  );
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    const payload = { id: u.id, username: u.username, name: u.name, role: u.role, exp: Date.now() + SESSION_MAX_AGE_MS };
...
app.get('/api/users', requireAuth, requireAdmin, async (req, res) => {
  const r = await pool.query(`SELECT id, username, name, role, created_at FROM ${USERS_TABLE} ORDER BY created_at ASC`);
...
app.delete('/api/users/:id', requireAuth, requireAdmin, async (req, res) => {
  try {
    const countR = await pool.query(`SELECT count(*)::int AS n FROM ${USERS_TABLE} WHERE role = 'admin'`);
    const target = await pool.query(`SELECT username, role FROM ${USERS_TABLE} WHERE id = $1`, [req.params.id]);
    ...
    await pool.query(`DELETE FROM ${USERS_TABLE} WHERE id = $1`, [req.params.id]);
...
app.get('/api/audit', requireAuth, requireAdmin, async (req, res) => {
  ...
  const r = await pool.query(`SELECT id, at, username, name, summary FROM ${AUDIT_TABLE} ORDER BY at DESC LIMIT $1`, [limit]);
  res.json({ entries: r.rows });
});

/* ---- Numérotation séquentielle atomique (devis / factures) ---- */
...
app.get('/api/state', requireAuth, async (req, res) => {
  try {
    const r = await pool.query(`SELECT data FROM ${STATE_TABLE} WHERE id = $1`, [ROW_ID]);
...
async function saveState(req, res) {
  try {
    ...
    const prev = await pool.query(`SELECT data FROM ${STATE_TABLE} WHERE id = $1`, [ROW_ID]);
